...
{
    public class RoundedGroupBox8 : GroupBox
    {
        public string colorfondo { get; set; } = "#FFFFFF";
        public int transparencia { get; set; } = 128;
        public RoundedGroupBox8()
        {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            using (GraphicsPath path = RoundedRectangle(rect, 25))
            {
                e.Graphics.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.AntiAlias;
                using (Pen pen = new Pen(Color.FromArgb(transparencia,ColorTranslator.FromHtml(colorfondo)), 2))
                {
                    e.Graphics.DrawPath(pen, path);
                }
                using (SolidBrush brush = new SolidBrush(Color.FromArgb(transparencia, ColorTranslator.FromHtml(colorfondo))))
                {
